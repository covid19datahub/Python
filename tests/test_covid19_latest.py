...
class TestCovid19Latest(unittest.TestCase):
    _sourceless_attributes = ['key','id','key_google_mobility','key_apple_mobility','date',
                              'iso_numeric','iso_alpha_2','iso_alpha_3','administrative_area_level',
                              'administrative_area_level_3','administrative_area_level_2',
                              'administrative_area_level_1','gatherings_restrictions',
                              'stay_home_restrictions','currency']
    _numeric_attributes = ["tests","confirmed","recovered","deaths","hosp","vent","icu"]
    _constant_attributes = ["population","latitude","longitude"]
    _indicator_attributes = ["school_closing","cancel_events","contact_tracing","testing_policy",
                             "transport_closing","workplace_closing","information_campaigns","stringency_index",
                             "international_movement_restrictions","internal_movement_restrictions"]
    _src_attributes = ["iso_alpha_3","administrative_area_level","data_type","url","title","year",
                       "bibtype","author","institution","textVersion"]
    ...
    def _check_src(self, x, src):
        ...
        cols -= set(['key_alpha_2','key_numeric']) # adjust
        for col in cols:
            # empty columns ignored
            if x[col].isnull().all() or (x[col] == 0).all(): continue
